...
import type {
    ...
    CompressorData,
    CompressorErrors,
    CompressorFormErrors,
    CoolingErrors,
    CoolingFormData,
    CoolingMachineData,
    CoolingMachineErrors,
    OtherDeviceData,
    OtherDeviceErrors,
    OtherErrors,
    OtherFormData,
...
export function validateComplex(
    ...
    if (!payload.postal) {
        errors.postal =
            'Add meg a telephelyhez tartozó irányítószámot/települést!'
        hasError = true
    }
...
export function validateBuilding(
    ...
    if (!payload.complex) {
        errors.complex =
            'Válassz ki egy telephelyet, amihez az épület tartozik!'
        hasError = true
    }
...
export function validateVentilationSystem(payload: VentilationFormData) {
    ...
    if (!payload.servicedBuilding || payload.servicedBuilding.length === 0) {
        errors.servicedBuilding =
            'Add meg a légkezelő által kiszolgált épületeket!'
        hasError = true
    }
        ...
        payload.servicedBuilding.forEach((e: ServicedBuildingShort) => {
            if (!e.servicedSize || e.servicedSize === 0) {
                errors.servicedSizes = {
                    ...errors.servicedSizes,
                    [e.buildingId]:
                        'Add meg a kiszolgált területet az épületben!',
                }
                hasError = true
            }
        })
    ...
    if (payload.retriever !== VentilationHeatRetrievers.NONE) {
        if (
            !payload.retrieverYear ||
            payload.retrieverYear < 0 ||
...
export function validateCompressed(payload: CompressedFormData) {
    const errors: CompressorFormErrors = {
        name: '',
        pressure: '',
        compressor: [],
    }
    ...
    if (!payload.name || payload.name === '') {
        errors.name = 'Add meg a rendszer nevét!'
        hasError = true
    }

    if (!payload.pressure || payload.pressure <= 0) {
        errors.pressure = 'Add meg a rendszer nyomását!'
        hasError = true
    }
    ...
    payload.compressors.forEach((compressor: CompressorData) => {
        ...
        if (!compressor.compressorType || compressor.compressorType === '') {
            localErrors.compressorType = 'Add meg a kompresszor típusát!'
            localHasError = true
        }

        if (!compressor.standing || compressor.standing === '') {
            localErrors.standing = 'Add meg a kompresszorhoz tartozó mérést!'
            localHasError = true
        }

        if (!compressor.hours || compressor.hours <= 0) {
            localErrors.hours = 'Add meg a kompresszor éves terhelt üzemidejét!'
            localHasError = true
        }

        if (!compressor.nominalOutput || compressor.nominalOutput <= 0) {
            localErrors.nominalOutput =
                'Add meg a kompresszor névleges teljesítményét!'
            localHasError = true
        }

        if (!compressor.amount || compressor.amount <= 0) {
            localErrors.amount = 'Add meg a kompresszorok mennyiségét!'
            localHasError = true
        }

        errors.compressor.push(localHasError ? localErrors : 'none')
        if (localHasError) {
            hasError = true
        }
    })
...
export function validateSteam(payload: SteamFormData) {
    let hasError = false
    const errors: SteamErrors = {
        name: '',
        pressure: '',
        machines: [],
    }
    ...
    if (!payload.pressure || payload.pressure <= 0) {
        errors.pressure = 'Add meg a hálózati nyomást!'
        hasError = true
    }

    payload.machines.forEach((machine: SteamMachineData) => {
        let localHasError = false
        const localErrors: SteamMachineErrors = {
            standing: '',
            type: '',
            nominalOutput: '',
        }
        ...
        if (!machine.type || machine.type === '') {
            localHasError = true
            localErrors.type = 'Add meg a gőzfejlesztő típusát!'
        }

        if (!machine.nominalOutput || machine.nominalOutput <= 0) {
            localHasError = true
            localErrors.nominalOutput =
...
export function validateCooling(payload: CoolingFormData) {
    let hasError = false
    const errors: CoolingErrors = {
        name: '',
        machines: [],
    }
    ...
    payload.machines.forEach((machine: CoolingMachineData) => {
        let localHasError = false
        const localErrors: CoolingMachineErrors = {
            standing: '',
            type: '',
            nominalOutput: '',
        }
        ...
        if (!machine.type || machine.type === '') {
            localHasError = true
            localErrors.type = 'Add meg a hűtőberendezés típusát!'
        }
...
export function validateOther(payload: OtherFormData) {
    let hasError = false
    ...
    payload.machines.forEach((machine: OtherDeviceData) => {
        let localHasError = false
        const localErrors: OtherDeviceErrors = {
            standing: '',
            type: '',
            nominalOutput: '',
            hours: '',
        ...
        if (!machine.type || machine.type === '') {
            localHasError = true
            localErrors.type = 'Add meg a berendezés típusát!'
        }
        ...
        if (!machine.hours || machine.hours <= 0) {
            localHasError = true
            localErrors.hours = 'Add meg a berendezés becsült éves üzemidejét!'
        }

        if (!machine.amount || machine.amount <= 0) {
            localHasError = true
            localErrors.amount = 'Add meg a berendezések számát!'
        }
